...
def get_combined_message_counts(dir_path):
    ...
    combined_message_counts = defaultdict(int)
    for file_path in txt_files:
        lines = load_kakao_chat(file_path)
        message_counts = extract_message_data(lines)
        for sender, count in message_counts.items():
            combined_message_counts[sender] += count
    return combined_message_counts

def display_message_counts(message_counts):
    sorted_counts = sorted(message_counts.items(), key=lambda x: x[1], reverse=True)
    # result = "=== 메시지 수 ===\n"
    result = ""
    for idx, data in enumerate(sorted_counts):
        sender, count = data[0], data[1]
        if sender in chat_bot:
            continue
        result += f'{idx+1:>2}. {sender}: {count} messages\n'
    return result
